...
class LostItem(db.Model):
    ...
    #? t = (10, "hello", 3.14)
    __tablename__ = 'students'  #* Optional, auto-created if not defined
...
def login():
    if request.method == 'POST':
        username = request.form.get('username', '').strip() #! remove space and \n
        password = request.form.get('password', '').strip()
        # Use OR here: if either username or password is incorrect, show error.
        if username != 'a' or password != 'a':
            return render_template('index.html', flag=0)  # show login with error
        # Successful login - redirect to the home page which handles DB display
        return redirect('/home')

    # GET: render the login page (index.html contains the login form)
    ...
    if search_query:
        ...
        if search_by == 'id':
                try:
                    ...
                    return render_template('home.html', all_items=[])
...
def faq():
    """FAQ / Query submission page.

    GET: Render the FAQ page with submission form.
    POST: Save the submitted query into instance/queries as a text file and show a confirmation.
    """
    if request.method == 'POST':
        ...
        # Ensure queries directory exists under the instance path
        queries_dir = os.path.join(app.config.get('INSTANCE_PATH', 'instance'), 'queries')
        os.makedirs(queries_dir, exist_ok=True)

        # Create a timestamped, unique filename
        timestamp = datetime.now(IST).strftime('%Y%m%d_%H%M%S')
        filename = f"query_{timestamp}_{uuid4().hex}.txt"
        filepath = os.path.join(queries_dir, filename)

        # Write the query to a plain text file (utf-8)
        try:
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(f"Timestamp: {datetime.now(IST).isoformat()}\n")
                f.write(f"Name: {name}\n")
            ...
            # Log the event
            log_message('INFO', f'FAQ query saved: {filename} by {email or name}')
            # After saving, fall through to build the recent_queries list and render
            saved_flag = True
            saved_filename = filename
        except Exception as e:
            log_message('ERROR', f'Failed to save FAQ query: {e}')
            # If saving failed, still show recent queries and display error
            saved_flag = False
            saved_filename = None
            save_error = str(e)
    else:
        # GET request; no save performed
        saved_flag = False
        saved_filename = None
        save_error = None

    # Build a list of recent queries from the instance/queries folder (most recent first)
    recent_queries = []
    try:
        files = []
        ...
        for fname, fpath, mtime in files[:5]:
            try:
                ...
                recent_queries.append({
                    'filename': fname,
                    'content': content,
                    'mtime': datetime.fromtimestamp(mtime, IST).strftime('%Y-%m-%d %H:%M') if mtime else ''
                })
            except Exception:
                continue
    except Exception:
    ...
    return render_template('faq.html', saved=saved_flag, filename=saved_filename if saved_flag else None, error=save_error if not saved_flag else None, recent_queries=recent_queries)
